...
if (Test-Path $InstallDir){
  ...
  if ($copyConfig) {
    ...
    else { Write-Host "$InstallDir\Views saknas" -BackgroundColor Red }
  ...
  if ($folderDiff.Length -eq 0) { Write-Host Käll- och dest-mappar för webbappen är identiska -BackgroundColor Yellow -ForegroundColor Black}
  else { Write-Host $folderDiff.Path -BackgroundColor Yellow -ForegroundColor Black }
